...
All the below calculations are based on the 
http://www.d.umn.edu/~gshute/net/delays-losses.xhtml
 * ****************************************************************************************
 * LAN Delay
 ...
 * ****************************************************************************************
 */
public class NetworkTopography implements NetworkConstants{
     ...
     * @return
     */
    public double calculateNetworkDelay(){
     ...
     * @return
     */
    public double  getLAN_Delay(double loadFactor, double dataSizeinGB){ // for light loaded loadFactor is 0.1 and heavily loaded loadFactor is 0.5

        d_trans = LAN_TRANSMISSION_RATE ; // millisec for 1GB
        d_trans = d_trans*dataSizeinGB; //millisec for real loads

        d_prop = LAN_PROPAGATION_DELAY; //millisec
       // loadFactor = 0.1;

        if (loadFactor == LAN_LOW_LOADFACTOR)
        d_queue = 0.5 * d_trans; // smaller than transmission delay
        else  if (loadFactor == LAN_HIGH_LOADFACTOR){ // heavily loaded
            d_queue = 2.5 * d_trans;
        }

        d_proc = LAN_PROCESSING_DELAY; // for modern networks this value is not significant
    ...
    public double getWAN_Delay(double loadFactor, double dataSizeinGB){// for heavily loaded loadFactor is 1.0

        d_trans =  WAN_TRANSMISSION_RATE; // millisec for 1GB
        d_trans = d_trans*dataSizeinGB; //millisec for real loads

        d_prop =  WAN_PROPAGATION_DELAY; // millisec

        if (loadFactor == WAN_LOW_LOADFACTOR)
            d_queue = d_prop; // compared to propagation delay
        else  if (loadFactor == WAN_HIGH_LOADFACTOR) { // heavily loaded
            d_queue = d_trans*0.5;
        }

        d_proc = WAN_PROCESSING_DELAY;
        return  calculateNetworkDelay();
